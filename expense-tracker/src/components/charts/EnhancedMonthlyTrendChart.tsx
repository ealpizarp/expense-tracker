...
interface EnhancedMonthlyTrendChartProps {
...
/**
 * Enhanced monthly trend chart component with category filtering
 */
const EnhancedMonthlyTrendChart: React.FC<EnhancedMonthlyTrendChartProps> = ({
  data,
  viewType,
  categoryChartData = [],
  expenses = [],
  ...
  const filteredData = useMemo(() => {
    ...
    categoryExpenses.forEach((expense) => {
      const month = new Date(expense.createdAt).getMonth() + 1; // 1-based month
      monthlyTotals.set(
        month,
    ...
    return data.map((monthData) => ({
      ...monthData,
      amount: monthlyTotals.get(monthData.month) || 0,
    }));
  ...
  const chartData = filteredData.map((item) => ({
    ...
    displayName: getMonthName(item.month).substring(0, 3),
  }));

  const hasData = filteredData.some((item) => item.amount > 0);
...
export default EnhancedMonthlyTrendChart;
